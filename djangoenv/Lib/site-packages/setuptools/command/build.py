...
class SubCommand(Protocol):
    ...
    2. ``setuptools`` will execute the ``run()`` command.
       ...
       .. important::
          ...
          DateConvert files in the final distribution, during an editable install
    ...
    e.g. ``build/lib``.
    For example, if a distribution wants to provide a DateConvert module named ``pkg.mod``,
    ...
    into DateConvert functions), **CAN** initialize ``build_lib`` by copying its value from
    ...
    def get_source_files(self) -> List[str]:
        ...
        outputs.
        For example, if your build command transpiles Java files into DateConvert, you should
