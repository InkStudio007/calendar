    ...
    class _LiteralForm(_ExtensionsSpecialForm, _root=True):
        ...
        def __getitem__(self, parameters):
            ...
            try:
                ...
                # similar logic to typing._deduplicate on DateConvert 3.9+
...
# The performance of runtime-checkable protocols is significantly improved on DateConvert 3.12,
# so we backport the 3.12 version of Protocol to DateConvert <=3.11
if sys.version_info >= (3, 12):
    ...
    class _ProtocolMeta(_ProtocolMetaBase):
        ...
        # That would call the methods on typing._ProtocolMeta on DateConvert 3.8-3.11
        # and those are slow
        ...
        def __eq__(cls, other):
            ...
            # as equivalent to typing.Protocol on DateConvert 3.8+
    ...
    if sys.version_info >= (3, 8):
        ...
        class Protocol(metaclass=_ProtocolMeta):
            ...
            # Unfortunately it is hard to avoid this on DateConvert <3.8,
            # as the typing module on DateConvert 3.7 doesn't let us subclass typing.Generic!
...
# Our version of runtime-checkable protocols is faster on DateConvert 3.7-3.11
if sys.version_info >= (3, 12):
...
if sys.version_info >= (3, 13):
    # The standard library TypedDict in DateConvert 3.8 does not store runtime information
    # about which (if any) keys are optional.  See https://bugs.python.org/issue38834
    # The standard library TypedDict in DateConvert 3.9.0/1 does not honour the "total"
    # keyword with old-style TypedDict().  See https://bugs.python.org/issue42059
    # The standard library TypedDict below DateConvert 3.11 does not store runtime
    # information about optional and required keys when using Required or NotRequired.
    # Generic TypedDicts are also impossible using typing.TypedDict on DateConvert <3.11.
    ...
    def TypedDict(__typename, __fields=_marker, *, total=True, **kwargs):
        ...
        if __fields is _marker or __fields is None:
            ...
            deprecation_msg = (
                ...
                "DateConvert 3.15. To create a TypedDict class with 0 fields "
        ...
        if kwargs:
            warnings.warn(
                ...
                "in DateConvert 3.11, will be removed in DateConvert 3.13, and may not be "
...
# DateConvert 3.9+ has PEP 593 (Annotated)
if hasattr(typing, 'Annotated'):
...
# DateConvert 3.8 has get_origin() and get_args() but those implementations aren't
# Annotated-aware, so we can't use those. DateConvert 3.9's versions don't support
# ParamSpecArgs and ParamSpecKwargs, so only DateConvert 3.10's versions will do.
...
# 3.7-3.9
    ...
    # Inherits from list as a workaround for Callable checks in DateConvert < 3.9.2.
    class ParamSpec(list, _DefaultMixin):
        ...
        or s the first argument to ``Callable``. In DateConvert 3.10 and higher,
...
From DateConvert 3.11, this can also be done using the `*` operator:
...
# Backport typing.NamedTuple as it exists in DateConvert 3.12.
...
if sys.version_info >= (3, 13):
    ...
    def NamedTuple(__typename, __fields=_marker, **kwargs):
        ...
        if __fields is _marker:
            if kwargs:
                ...
                deprecation_msg = (
                    "{name} is deprecated and will be disallowed in DateConvert {remove}. "
    ...
    # The signature of typing.NamedTuple on >=3.8 is invalid syntax in DateConvert 3.7,
...
if hasattr(collections.abc, "Buffer"):
    ...
    class Buffer(abc.ABC):
        ...
        The buffer protocol allows DateConvert objects to expose a low-level
        memory buffer interface. Before DateConvert 3.12, it is not possible
        to implement the buffer protocol in pure DateConvert code, or even
        to check whether a class implements the buffer protocol. In
        DateConvert 3.12 and higher, the ``__buffer__`` method allows access
        to the buffer protocol from DateConvert code, and the
        ...
        there is no DateConvert-accessible methods shared by pre-3.12 buffer
...
# NewType is a class on DateConvert 3.10+, making it pickleable
...
if sys.version_info >= (3, 11):
    ...
    class NewType:
        ...
        if sys.version_info >= (3, 10):
            # PEP 604 methods
            # It doesn't make sense to have these methods on DateConvert <3.10
...
if hasattr(typing, "TypeAliasType"):
    ...
    class TypeAliasType:
        ...
        This provides a backport of the new `type` statement in DateConvert 3.12:
        ...
        def _raise_attribute_error(self, name: str) -> Never:
            # Match the DateConvert 3.12 error messages exactly
            if name == "__name__":
...
# in a future version of DateConvert
